...
class KubeCacheFS(Operations):
    ...
    def __init__(self, storage_path, log_path):
    ...
    def rmdir(self, path):
        self.log.write("rmdir,{}\n".format(self._full_path(path)))
        full_path = self._full_path(path)
    ...
    def rename(self, old, new):
        self.log.write("rename,{},{}\n".format(self._full_path(old), self._full_path(new)))
    ...
    def open(self, path, flags):
        full_path = self._full_path(path)
        self.log.write("open,{},{}\n".format(full_path, flags))
        return os.open(full_path, flags)

    def create(self, path, mode, fi=None):
        full_path = self._full_path(path)
        self.log.write("create,{},{}\n".format(full_path, mode))
    ...
    def write(self, path, buf, offset, fh):
        self.log.write("write,{},{},{}\n".format(self._full_path(path), len(buf), offset))
        os.lseek(fh, offset, os.SEEK_SET)
        return os.write(fh, buf)

    def truncate(self, path, length, fh=None):
        full_path = self._full_path(path)
        self.log.write("fsync,{}\n".format(full_path, length))
...
def main(mountpoint, root, log_path):
    FUSE(KubeCacheFS(root, log_path), 
        mountpoint, 
        nothreads=True, 
        foreground=True, 
        allow_other=True)
