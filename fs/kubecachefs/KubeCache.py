...
class KubeCache:
    ...
    def _get_cache_list_from_config(config):
        ...
        for cache in config["caches"]:
            if cache["replacement_policy"]=="LRU":
                cache_list.append(LRU(cache["size"]*256))
            # elif cache["replacement_policy"]=="LFU":
            #     cache_list.append(LFU(cache["size"]))
        return cache_list 
    ...
    def read(self, path, length, offset, fh):      
        """ Process a read request. Check if the requested data is in the cache, if it is 
            return from the cache if not read from the underlying FS and insert data into 
            the I/O cache. """
